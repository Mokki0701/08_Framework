...
public class EditBoardServiceImpl implements EditBoardService {
	...
	public int boardInsert(Board inputBoard, List<MultipartFile> images) throws IllegalStateException, IOException {
		...
		// images 리스트에서 하나씩 꺼내어 업로드된 파일이 있는지 검사
		...
		// 2) 여러 행을 삽입하는 SQL을 호출 (사용!!!!!!!!)
		
		// result == 삽입된 행의 개수 == uploadList.size();
		result = mapper.insertImageUploadList(uploadList);
		...
		if(result == uploadList.size()) {
			...
			// -> rollback 하는 방법 == 강제로 RuntimeException 강제 발생 (@Transactional)
