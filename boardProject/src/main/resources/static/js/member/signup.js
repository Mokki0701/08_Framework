...
// 필수 입력 항목의 유혀성 검사 여부를 체크하기 위한 객체
...
const checkPw = () => {
    ...
    pwMessage.innerText = "비밀번호가 잎치하지 않습니다.";
...
// 2) 먼저 memberPw먼저 유효성 검사 진행 -> 그거에 따른 pwMessage 값, 색 변경
// 비어있는경우, 유효성 검사에 맞지 않을 경우, 
memberPw.addEventListener("input", e=>{
    ...
    if(inputPw.trim().length === 0){ // 패스워드를 입력하지 않은 경우
    ...
    const regExp = /^[a-zA-Z0-9!@#\-_]{6,20}$/; // 정규식
    if(!regExp.test(inputPw)){ // 입력된 패스워드가 정규식 검사에 맞지 않을 경우
        pwMessage.innerText = "패스워드 조건에 맞게 입력해주세요.";
        pwMessage.classList.add("error");
        pwMessage.classList.remove("confirm");
    ...
    pwMessage.innerText="유효한 형식의 비밀번호 입니다.";
    pwMessage.classList.add("confirm");
    pwMessage.classList.remove("error");
...
memberPwConfirm.addEventListener("input", e=>{
    ...
    if(checkObj.memberPw){ // memberPw가 유효한
        checkPw();
        return;
    }
...
memberNickname.addEventListener("input", e=>{
    ...
    const inputNickname = memberNickname.value;
    ...
    if(!regExp.test(inputNickname)){
        nickMessage.innerText = "닉네임 조건에 부합되지 않습니다.";
    ...
    .then(resp => resp.text())
    .then(result => {
        if(result == 1){ // 닉네임이 중복된 경우
            nickMessage.innerText = "중복된 닉네임이 존재합니다.";
            nickMessage.classList.add("error");
            nickMessage.classList.remove("confirm");
        ...
        nickMessage.innerText = "닉네임 사용이 가능합니다.";
...
memberTel.addEventListener("input", e=>{
    ...
    telMessage.innerText = "전화번호 형식에 맞습니다.";
...
//    같으면 1, 아니면 0 반환
// 단, 타이머가 00:00초가 아닐경우에만 수행
checkAuthKeyBtn.addEventListener("click", () => {

    if(min === 0 && sec === 0){ // 타이머가 00:00 dls ruddn
